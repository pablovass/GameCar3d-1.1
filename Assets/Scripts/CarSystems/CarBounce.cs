    ...
    public class CarBounce : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            for (int i = 0; i < Angles.Length; i++)
            {
                ...
                if (Physics.Raycast(origin, direction, out RaycastHit hit, RayDistance, CollisionLayer) && Time.time > resumeTime && !hasCollided && car.LocalSpeed() > 0)
                {
                    ...
                    car.Rigidbody.velocity /= 2;
                    // Apply the bounce impulse with the reflectionVector
                    car.Rigidbody.AddForce(reflectionVector.normalized * BounceFactor, ForceMode.Impulse);
